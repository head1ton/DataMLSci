...
st.set_page_config(page_title='The Machine Learning Hyperparameter Optimization', layout='wide')

st.title("""
# The Machine Learning Hyperparamter Optimization App
**(Regression Edition)**
...
def build_model(df):
    ...
    # Segment data info groups based on the 2 hyperparameters
    grid_contour = grid_results.groupby(['max_features', 'n_estimators']).mean()
    # Pivoting the data
    grid_reset = grid_contour.reset_index()
    grid_reset.columns = ['max_features', 'n_estimators', 'R2']
    # grid_pivot = grid_reset.pivot('max_features', 'n_estimators')
    # Pivoting the data safely using pivot_table
    grid_pivot = grid_reset.pivot_table(values='R2', index='max_features',
                                        columns='n_estimators', aggfunc='mean')
    x = grid_pivot.columns.values
    y = grid_pivot.index.values
    z = grid_pivot.values

    layout = go.Layout(xaxis=go.layout.XAxis(title=go.layout.xaxis.Title(text='n_estimators')))
